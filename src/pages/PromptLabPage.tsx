    ...
    <div className="min-h-screen bg-theme-bg text-theme-950">
      ...
      <main className="mx-auto max-w-7xl px-5 py-8 md:px-8 md:py-10">
        <div className="grid gap-8 lg:grid-cols-[21rem_minmax(0,1fr)] xl:gap-10">
          <section>
            <p className="text-xs font-semibold text-teal-400">DAILY PROMPT LAB</p>
            ...
            <div className="mt-7 space-y-6">
              <fieldset>
                ...
                <div className="mt-2 inline-flex overflow-hidden rounded-lg border border-white/10 bg-white/5">
                  {([{ key: 'male', label: '男频' }, { key: 'female', label: '女频' }] as const).map((item) => (
                    <button key={item.key} type="button" onClick={() => { setChannel(item.key); setGenre('auto'); setKeyword('auto'); setSeed(0); setPosterError('') }} className={`min-h-10 px-5 text-sm ${channel === item.key ? 'bg-theme-600 text-white' : 'text-slate-300 hover:bg-white/10'}`}>
                      {item.label}
                    </button>
                  ))}
                </div>
              ...
              <button type="button" onClick={drawAgain} disabled={!prompt} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-theme-800 px-5 py-2.5 text-sm font-semibold text-white hover:bg-theme-600 disabled:opacity-50">
          ...
          <section aria-live="polite" className="min-w-0">
            {prompt ? (
              <article className="glass">
                <header className="border-b border-white/10 bg-white/5 px-5 py-5 sm:px-7 sm:py-6">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <p className="text-xs font-semibold text-theme-500">NO. {String(seed + 1).padStart(3, '0')} · {data?.updatedAt}</p>
                    <div className="flex flex-wrap gap-2">
                      <span className="rounded-full bg-white/10 px-2.5 py-1 text-xs font-semibold text-theme-700">{prompt.genre}</span>
                      <span className="rounded-full bg-teal-400/10 px-2.5 py-1 text-xs font-semibold text-teal-300">{prompt.keyword}</span>
                    </div>
                  </div>
                ...
                <div className="px-5 py-6 sm:px-7">
                  <section className="grid gap-x-6 gap-y-5 sm:grid-cols-2">
                    <div className="border-l-2 border-blue-500 pl-4">
                      <p className="flex items-center gap-2 text-xs font-semibold text-blue-300"><Users size={15} />主角与关系</p>
                    ...
                    <div className="border-l-2 border-emerald-500 pl-4">
                      <p className="flex items-center gap-2 text-xs font-semibold text-emerald-300"><Target size={15} />目标</p>
                      <p className="mt-2 text-sm font-semibold leading-relaxed">{prompt.goal}</p>
                    </div>
                    <div className="border-l-2 border-amber-500 pl-4">
                      <p className="text-xs font-semibold text-amber-300">障碍与失败代价</p>
                    ...
                    <div className="border-l-2 border-violet-500 pl-4">
                      <p className="flex items-center gap-2 text-xs font-semibold text-violet-300"><Eye size={15} />隐藏真相</p>
                    ...
                    <ol className="mt-4 divide-y divide-white/10 border-y border-white/10">
                      {prompt.chapters.map((chapter, index) => (
                        <li key={chapter.label} className="grid gap-3 py-5 sm:grid-cols-[3rem_minmax(0,1fr)]">
                          ...
                          <div>
                            <h4 className="text-sm font-bold text-theme-950">{chapter.label}</h4>
                            <p className="mt-2 text-sm leading-relaxed text-theme-700">{chapter.beat}</p>
                            <p className="mt-2 text-xs font-medium leading-relaxed text-teal-300">{chapter.cliffhanger}</p>
                          </div>
                        </li>
                      ))}
                    </ol>
                ...
                <footer className="flex flex-wrap justify-end gap-2 border-t border-white/10 bg-white/5 px-5 py-4 sm:px-7">
                  <button type="button" onClick={copyPrompt} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-theme-700 hover:bg-white/10">
                    {copied ? <Check size={16} /> : <Copy size={16} />} {copied ? '命题已复制' : '复制命题'}
                  </button>
                  <button type="button" onClick={sharePrompt} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-teal-400/30 bg-white/5 px-4 py-2 text-sm font-medium text-teal-300 hover:bg-white/10">
                    <Share2 size={16} /> 分享文本
                  </button>
