      ...
      <main className="mx-auto max-w-5xl px-5 py-8 md:px-8 md:py-10">
        <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
          <section>
            ...
            <p className="mt-2 text-sm leading-relaxed text-theme-500">选择“跟随今日风向”时，每次抽取都会从当天榜单和关键词中重新组合。</p>
            ...
            <div className="mt-7 space-y-6">
              <fieldset>
                ...
                <div className="mt-2 inline-flex overflow-hidden rounded-lg border border-theme-200 bg-white">
                  {([{ key: 'male', label: '男频' }, { key: 'female', label: '女频' }] as const).map((item) => (
                    <button key={item.key} type="button" onClick={() => { setChannel(item.key); setGenre('auto'); setKeyword('auto'); setSeed(0) }} className={`min-h-10 px-5 text-sm ${channel === item.key ? 'bg-theme-600 text-white' : 'text-theme-600 hover:bg-theme-50'}`}>
                      {item.label}
                    </button>
                  ))}
                </div>
              ...
              <label className="block text-sm font-semibold">题材
                <select value={genre} onChange={(event) => setGenre(event.target.value)} className="mt-2 min-h-11 w-full rounded-lg border border-theme-200 bg-white px-3 text-sm font-normal outline-none focus:border-theme-400">
              ...
              <label className="block text-sm font-semibold">核心关键词
                <select value={keyword} onChange={(event) => setKeyword(event.target.value)} className="mt-2 min-h-11 w-full rounded-lg border border-theme-200 bg-white px-3 text-sm font-normal outline-none focus:border-theme-400">
          ...
          <section aria-live="polite">
            {prompt ? (
              <div className="border border-theme-100 bg-white shadow-sm">
                <div className="border-b border-theme-100 bg-[hsl(var(--theme-50))] px-5 py-4 sm:px-7">
                ...
                <div className="space-y-5 p-5 sm:p-7">
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="border-l-2 border-theme-500 pl-3"><p className="text-xs text-theme-600">题材</p><p className="mt-1 text-sm font-semibold">{prompt.genre}</p></div>
                    <div className="border-l-2 border-teal-600 pl-3"><p className="text-xs text-theme-600">风向关键词</p><p className="mt-1 text-sm font-semibold">{prompt.keyword}</p></div>
                    <div className="border-l-2 border-blue-600 pl-3"><p className="text-xs text-theme-600">主角人设</p><p className="mt-1 text-sm font-semibold">{prompt.persona}</p></div>
                    <div className="border-l-2 border-amber-500 pl-3"><p className="text-xs text-theme-600">核心爽点</p><p className="mt-1 text-sm font-semibold">{prompt.trope}</p></div>
                  </div>
                  <div className="border-t border-theme-100 pt-5"><p className="text-xs font-semibold text-theme-600">一句话梗概</p><p className="mt-2 text-sm leading-relaxed text-theme-800">{prompt.pitch}</p></div>
                  <div><p className="text-xs font-semibold text-theme-600">黄金三章任务</p><p className="mt-2 text-sm leading-relaxed text-theme-800">{prompt.opening}</p></div>
                </div>
                <div className="flex flex-wrap justify-end gap-2 border-t border-theme-100 px-5 py-4 sm:px-7">
            ...
            ) : <p className="text-sm text-theme-600">风向数据加载中…</p>}
          </section>
        </div>
