    ...
    <div className="min-h-screen bg-[#fff7f8] text-rose-950">
      ...
      <main className="mx-auto max-w-6xl px-5 py-8 md:px-8 md:py-10">
        <div className="grid gap-8 lg:grid-cols-[0.82fr_1.18fr]">
          <section aria-labelledby="radar-form-title">
            ...
            <div className="mt-7 space-y-6">
              <fieldset>
                ...
                <div className="mt-2 inline-flex overflow-hidden rounded-lg border border-rose-200 bg-white">
                  {([{ key: 'male', label: '男频' }, { key: 'female', label: '女频' }] as const).map((item) => (
                    <button key={item.key} type="button" onClick={() => { setChannel(item.key); setGenreName(''); setGenerated(false) }} className={`min-h-10 px-5 text-sm ${channel === item.key ? 'bg-rose-600 text-white' : 'text-rose-600 hover:bg-rose-50'}`}>
                      {item.label}
                    </button>
                  ))}
                </div>
              ...
              <label className="block text-sm font-semibold">
                想写的题材
                <select value={selectedName} onChange={(event) => { setGenreName(event.target.value); setGenerated(false) }} className="mt-2 min-h-11 w-full rounded-lg border border-rose-200 bg-white px-3 text-sm font-normal text-rose-900 outline-none focus:border-rose-400">
              ...
              <fieldset>
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  {([{ key: 'sprint', label: '冲刺型' }, { key: 'steady', label: '稳定型' }, { key: 'slow', label: '慢工型' }] as const).map((item) => (
                    <button key={item.key} type="button" onClick={() => { setPace(item.key); setGenerated(false) }} className={`min-h-10 rounded-lg border px-2 text-sm ${pace === item.key ? 'border-teal-700 bg-teal-700 text-white' : 'border-rose-200 bg-white text-rose-600 hover:bg-rose-50'}`}>
                      {item.label}
                    </button>
                  ))}
                </div>
              ...
              <label className="block text-sm font-semibold">
                计划篇幅
                <select value={length} onChange={(event) => { setLength(event.target.value as Length); setGenerated(false) }} className="mt-2 min-h-11 w-full rounded-lg border border-rose-200 bg-white px-3 text-sm font-normal text-rose-900 outline-none focus:border-rose-400">
              ...
              <label className="flex items-center gap-3 text-sm font-semibold">
                <input type="checkbox" checked={adaptation} onChange={(event) => { setAdaptation(event.target.checked); setGenerated(false) }} className="h-4 w-4 accent-rose-600" />
                考虑短剧 / 漫剧改编
              </label>
          ...
          <section aria-live="polite" className="min-h-[34rem] border-l-0 border-rose-100 lg:border-l lg:pl-8">
            {!generated || !report || !selected ? (
              <div className="flex min-h-[34rem] items-center justify-center border border-dashed border-rose-200 bg-white/50 p-8 text-center">
                <div>
                  ...
                  <p className="mt-2 max-w-sm text-sm leading-relaxed text-rose-400">包含适配度、趋势阶段、拥挤度、开篇切口和更新策略。</p>
                </div>
              </div>
            ) : (
              <div className="border border-rose-100 bg-white p-5 shadow-sm sm:p-7">
                <div className="flex flex-wrap items-start justify-between gap-4 border-b border-rose-100 pb-6">
                  <div>
                    <p className="text-xs font-semibold text-rose-400">你的下一本</p>
                ...
                <div className="grid gap-px border-b border-rose-100 bg-rose-100 sm:grid-cols-3">
                  <div className="bg-white py-4 sm:pr-3"><p className="text-xs text-rose-400">7 日变化</p><p className="mt-1 font-mono text-lg font-bold text-emerald-700">{formatDelta(selected.delta7)}</p></div>
                  <div className="bg-white py-4 sm:px-3"><p className="text-xs text-rose-400">趋势加速度</p><p className="mt-1 font-mono text-lg font-bold text-blue-700">{formatDelta(selected.acceleration)}</p></div>
                  <div className="bg-white py-4 sm:pl-3"><p className="text-xs text-rose-400">竞争拥挤度</p><p className="mt-1 font-mono text-lg font-bold text-amber-700">{selected.crowding}</p></div>
                </div>

                <div className="mt-6 space-y-5">
                  <div className="flex gap-3"><TrendingUp className="mt-0.5 flex-none text-emerald-600" size={18} /><div><h3 className="text-sm font-bold">结论：{report.verdict}</h3><p className="mt-1 text-sm leading-relaxed text-rose-600">{selected.advice}</p></div></div>
                  <div className="flex gap-3"><Sparkles className="mt-0.5 flex-none text-blue-600" size={18} /><div><h3 className="text-sm font-bold">开篇切口</h3><p className="mt-1 text-sm leading-relaxed text-rose-600">{report.hook}</p></div></div>
                  <div className="flex gap-3"><Check className="mt-0.5 flex-none text-teal-700" size={18} /><div><h3 className="text-sm font-bold">执行建议</h3><p className="mt-1 text-sm leading-relaxed text-rose-600">{report.paceText} {report.adaptationText}</p>{report.personaName && <p className="mt-2 text-xs font-medium text-teal-700">已结合你的创作人格：{report.personaName}</p>}</div></div>
                </div>

                <div className="mt-7 flex flex-wrap items-center justify-between gap-3 border-t border-rose-100 pt-5">
