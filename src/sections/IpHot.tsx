...
const FORM_STYLE: Record<string, string> = {
  ...
  漫剧: 'bg-theme-200 text-theme-700',
        ...
        {data.categories.map((cat) => {
          ...
          >
            <div className="mb-4 flex items-center justify-between border-b border-theme-50 pb-3">
              ...
              <span className="text-xs text-theme-500">按热度降序</span>
            </div>
            <ol className="space-y-3">
              {sortedItems.map((it, index) => (
                <li key={`${cat.name}-${it.title}`} className="group flex items-start gap-3 text-sm">
                  <span className="mt-0.5 flex h-6 w-6 flex-none items-center justify-center rounded-lg bg-gradient-to-br from-theme-100 to-theme-200 text-xs font-bold text-theme-600 shadow-sm">
                    {index + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-theme-600">
