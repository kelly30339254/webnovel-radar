...
export default function TodayDecisions({ genres, history, boards }: { genres: GenreHeat[]; history: HistoryData | null; boards: Board[] }) {
  ...
  return (
    <section className="mt-12 border-y border-theme-200 bg-theme-50/55 px-4 py-6 sm:px-5" aria-labelledby="today-decisions-title">
      <SectionTitle id="today-decisions-title" title="今日三件事" hint="先看行动信号，再看完整榜单" />
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        {items.map((item) => {
          const Icon = item.icon
          return (
            <div key={item.label} className="rounded-lg border border-theme-200 bg-white p-4 shadow-sm sm:p-5">
