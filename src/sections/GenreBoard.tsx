        ...
        {signals.map((signal, index) => (
          <li key={signal.name} className="px-2 py-4 sm:px-3">
            <div className="grid grid-cols-[2rem_minmax(0,1fr)_3.2rem] items-start gap-2 sm:grid-cols-[2rem_minmax(10rem,1.2fr)_minmax(8rem,1fr)_4rem_4.5rem_4rem] sm:items-center sm:gap-3">
              <span className="pt-0.5 font-mono text-sm text-theme-500">{String(index + 1).padStart(2, '0')}</span>
              ...
              <div className="hidden text-center sm:block"><p className="font-mono text-sm font-semibold text-theme-900">{formatDelta(signal.delta7)}</p><p className="text-[10px] text-theme-600">7 日</p></div>
              <div className="hidden text-center sm:block"><p className="font-mono text-sm font-semibold text-amber-700">{signal.crowding}</p><p className="text-[10px] text-theme-600">拥挤度</p></div>
              <div className="text-right"><p className="font-mono text-base font-bold text-theme-700">{signal.heat}</p><p className="text-[10px] text-theme-600">热度</p></div>
              <div className="col-start-2 col-end-[-1] mt-2 grid grid-cols-3 gap-px bg-theme-100 sm:hidden">
                <div className="bg-white py-2"><p className="font-mono text-sm font-semibold text-emerald-700">{formatDelta(signal.delta7)}</p><p className="text-[10px] text-theme-600">7 日变化</p></div>
                <div className="bg-white py-2 text-center"><p className="font-mono text-sm font-semibold text-blue-700">{formatDelta(signal.acceleration)}</p><p className="text-[10px] text-theme-600">加速度</p></div>
                <div className="bg-white py-2 text-right"><p className="font-mono text-sm font-semibold text-amber-700">{signal.crowding}</p><p className="text-[10px] text-theme-600">拥挤度</p></div>
              </div>
            </div>
          </li>
        ))}
