...
import { createNbtiPoster, downloadBlob } from '@/lib/nbtiPoster'
        ...
        return (
          <div key={dimension.key}>
            <div className="flex items-center justify-between text-xs font-medium text-rose-900">
              ...
              <span className="text-rose-400">{100 - score}% {dimension.right}</span>
            </div>
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-rose-100" role="img" aria-label={`${dimension.left} ${score}%，${dimension.right} ${100 - score}%`}>
  ...
  return (
    <div>
      <div className="text-center">
        ...
        <p className="mt-3 text-xs font-semibold text-rose-400">{result}</p>
        <h2 className="mt-1 text-2xl font-bold text-rose-950 md:text-3xl">{info.name}</h2>
        <p className="mx-auto mt-3 max-w-lg text-sm leading-relaxed text-rose-700 md:text-base">{info.desc}</p>
      </div>
      ...
      <div className="mt-6 border-t border-rose-100 pt-5">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {info.genres.map((genre, index) => (
            <span
              key={genre}
              className={`rounded-full px-2.5 py-1 text-xs font-medium ${index === 1 ? 'bg-teal-50 text-teal-700' : index === 2 ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700'}`}
            >
              {genre}
            </span>
          ))}
        ...
        <ul className="mt-2 grid gap-1 text-xs text-rose-600 sm:grid-cols-2">
      ...
      <div className="mt-6 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-center">
        <button onClick={handlePoster} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-lg bg-rose-600 px-4 py-2 text-sm font-medium text-white hover:bg-rose-700">
          <Download size={16} />
        ...
        <button onClick={handleCopy} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-lg border border-rose-200 bg-white px-4 py-2 text-sm font-medium text-rose-700 hover:bg-rose-50">
      ...
      <div className="mt-4 text-center">
        <button onClick={onReset} className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-rose-500 hover:text-rose-700">
...
export default function NbtiTester({ standalone = false, initialResult }: { standalone?: boolean; initialResult?: ResultKey | null }) {
  ...
  return (
    <section className={`${standalone ? 'min-h-[70vh] py-4' : 'mt-10'}`}>
      ...
      <div className={`card-pink border border-rose-100 bg-white/80 p-5 shadow-sm backdrop-blur-sm sm:p-7 ${standalone ? 'mx-auto max-w-2xl rounded-lg' : 'mt-4 rounded-lg'}`}>
        {!started ? (
          <div className="py-3 text-center">
            <p className="text-4xl">📝</p>
            <h2 className="mt-3 text-lg font-bold text-rose-950">60 秒测出你的网文创作人格</h2>
            <p className="mt-1 text-sm text-rose-500">20 道二选一，看看你是爆更派、脑洞派，还是精密鸽王</p>
            <button
              onClick={() => {
                setStarted(true)
                trackEvent('nbti_start')
              }}
              className="mt-5 inline-flex min-h-10 items-center justify-center gap-1 rounded-lg bg-rose-600 px-6 py-2 text-sm font-medium text-white shadow-sm hover:bg-rose-700"
            >
          ...
          <div>
            <div className="mb-4 flex items-center justify-between text-xs text-rose-500">
              <span>第 {index + 1} / {NBTI_QUESTIONS.length} 题</span>
              <span>{progress}%</span>
            </div>
            <div className="h-1.5 w-full overflow-hidden rounded-full bg-rose-100">
              <div className="h-full rounded-full bg-rose-500 transition-all duration-300" style={{ width: `${progress}%` }} />
            </div>
            <p className="mt-6 text-lg font-semibold text-rose-950">{current.text}</p>
            <div className="mt-5 grid gap-3">
              {current.options.map((option, optionIndex) => (
                <button
                  key={option}
                  onClick={() => handleAnswer(optionIndex)}
                  className="min-h-12 rounded-lg border border-rose-100 bg-white px-4 py-3 text-left text-sm text-rose-800 shadow-sm hover:border-rose-300 hover:bg-rose-50"
                >
                  {option}
                </button>
              ))}
