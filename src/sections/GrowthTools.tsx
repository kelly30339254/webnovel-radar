    ...
    <section className="mt-10" aria-labelledby="growth-tools-title">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        {TOOLS.map((tool) => {
          ...
          return (
            <Link key={tool.to} to={tool.to} onClick={() => trackEvent(tool.event)} className="group flex min-h-32 items-start gap-4 border border-rose-100 bg-white p-4 shadow-sm hover:border-rose-200 hover:shadow-md">
              <span className={`flex h-10 w-10 flex-none items-center justify-center rounded-lg border ${tool.tone}`}><Icon size={19} /></span>
              <span className="min-w-0 flex-1">
                <span className="block text-base font-bold text-rose-950">{tool.title}</span>
                <span className="mt-1 block text-sm leading-relaxed text-rose-500">{tool.description}</span>
              </span>
              <ArrowRight className="mt-1 flex-none text-rose-300 transition-transform group-hover:translate-x-1 group-hover:text-rose-500" size={17} />
            </Link>
          )
        })}
      </div>
