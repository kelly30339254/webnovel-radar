import { ArrowRight, Dices, PenLine, SpellCheck, Target } from 'lucide-react'
    ...
    <section id="tools" className="mt-12 scroll-mt-24" aria-labelledby="growth-tools-title">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-12">
        ...
        {SECONDARY_TOOLS.map((tool) => {
          ...
          return (
            <Link key={tool.to} to={tool.to} onClick={() => trackEvent(tool.event)} className={`group relative flex min-h-72 flex-col overflow-hidden rounded-2xl border p-6 transition-all hover:-translate-y-1 hover:shadow-[0_18px_44px_-14px] hover:shadow-rose-500/20 lg:col-span-4 ${tool.surface}`}>
              <span className="absolute right-5 top-3 font-serif text-6xl font-black opacity-10" aria-hidden="true">{tool.num}</span>
