export default function Footer({ updatedAt, onEasterEgg }: { updatedAt: string; onEasterEgg?: () => void }) {
  return (
    <footer className="mt-16 border-t border-rose-100 bg-gradient-to-r from-rose-50 via-pink-50 to-rose-50">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-8 gap-y-2 px-5 py-6 text-xs text-rose-400 md:px-8">
        ...
        <span className="ml-auto inline-flex items-center gap-2">
          ...
          <button
            ...
            className="heart-beat inline-flex h-5 w-5 items-center justify-center rounded-full bg-rose-200/60 text-[9px] font-bold text-white opacity-60 transition-opacity hover:opacity-100"
            title="?"
            aria-label="?"
            type="button"
          >
