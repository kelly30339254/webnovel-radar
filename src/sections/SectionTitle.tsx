    ...
    <div className="relative pb-4">
      <div className="flex items-center gap-4">
        ...
        <div className="text-center">
          ...
          {hint && <p className="mt-1 text-[11px] tracking-[0.12em] text-slate-400">{hint}</p>}
        </div>
        <span className="h-px flex-1 bg-theme-400/70" aria-hidden="true" />
      </div>
      {right && <div className="mt-3 flex justify-center sm:absolute sm:right-0 sm:top-0 sm:mt-0">{right}</div>}
      {footer && <div className="mt-3 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-xs text-slate-400">{footer}</div>}
    </div>
  )
}
